...
class DataProviderService:
    ...
    def get_post_with_author(self, post_id=None):
        ...
        if post_id is None:
            sql_all_posts = "SELECT p.id, author_id, created, title, content, u.username FROM post as p inner join " \
                            "user_table as u on p.author_id = u.id  order by created desc "
            self.cursor.execute(sql_all_posts)
